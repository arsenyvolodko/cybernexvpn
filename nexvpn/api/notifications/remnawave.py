...
def _enforce_device_limit(panel_user_id: int, hwid: str | None) -> bool:
    ...
    На практике это мёртвый код: событие `user_hwid_devices.added`, на которое
    он реагирует, панель заявляет в своём API, но эмпирически (10.09.2026, 72+
    часов живого трафика с реальными добавлениями устройств) ни разу не
    прислала — только `.deleted` и `user.modified`. Настоящий запрет работает
    опросом, см. `nexvpn.tasks.enforce_device_limits` и
    `panel_sync.reject_devices_added_over_limit`. Эта функция оставлена на
    случай, если панель когда-нибудь всё же начнёт слать это событие — тогда
    сработает мгновенно, вместо того чтобы ждать следующего опроса; удалять
    её незачем, но и рассчитывать, что она сейчас что-то ловит, не стоит.
    """
    if not hwid:
