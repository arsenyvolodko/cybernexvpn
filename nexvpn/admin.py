...
class GlobalSettingsAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ...
        ("Реферальная программа", {
            "fields": ("referral_inviter_days", "referral_invitee_days"),
            "description": "Начисляются обоим после первой оплаты приглашённого. Суммы "
                           "разные для инвайтера и приглашённого — меняются независимо. "
                           "Действует сразу, без перезапуска: экран в боте и API "
                           "приглашения берут значение отсюда же.",
        }),
    )
    readonly_fields = ()
