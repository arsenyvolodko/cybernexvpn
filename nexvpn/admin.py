...
class GlobalSettingsAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ...
        ("Реферальная программа", {
            "fields": ("referral_inviter_days_min", "referral_inviter_days_max", "referral_invitee_days"),
            "description": "Начисляются обоим после первой оплаты приглашённого. Приглашённому — "
                           "фиксированные дни. Инвайтеру — «его тариф не дороже» (max), если на "
                           "тот момент его тариф (₽/мес) не дороже того, что оплатил приглашённый, "
                           "иначе «его тариф дороже» (min). Действует сразу, без перезапуска: "
                           "экран в боте и API приглашения берут значение отсюда же.",
        }),
    )
    readonly_fields = ()
