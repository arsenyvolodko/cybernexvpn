...
# Общий секрет со сборщиком на нодах. Пусто — приём телеметрии выключен:
# подделанная статистика тише сломанной, и разбираться в ней потом дороже.
NODE_TELEMETRY_TOKEN = env.str("NODE_TELEMETRY_TOKEN", "")
# Адрес входа московского релея. Соединения, пришедшие на европейскую ноду с
# него, — это профили «через РФ»; по инбаунду они от прямых неотличимы.
RELAY_NODE_ADDRESS = env.str("RELAY_NODE_ADDRESS", "ru1.pineferry.com")
...
# С какого момента новичкам шлём подталкивания «ты ещё не подключился».
# Отсечка нужна ради тех, кто открыл бота незадолго до выката: они попадают в
# суточное окно рассылки и без неё получили бы сообщение задним числом (на
# 11.09.2026 таких было двое). Кто заходил раньше — выпадает по самому окну.
# Значение = момент выката, ISO 8601 с зоной. Пусто — рассылка выключена.
ONBOARDING_NUDGES_SINCE = env.str("ONBOARDING_NUDGES_SINCE", "2026-09-11T23:40:05+03:00")
